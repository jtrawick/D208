...
init_mdl %>%
  glimpse()
```
We will next need to convert the dtypes of many our variables. Most of the variables which are currently of the type Character, and some that are of type Integer will need to be converted to Factors. This will ensure that the model handles the variables as intended.
...
# init_mdl$Gender <- as_factor(init_mdl$Gender)

# init_mdl[fctvars] <- lapply(init_mdl[fctvars], factor)
```

```{r}
# Reformat all chr variables as factors
